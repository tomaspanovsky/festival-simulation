...
Po nastavení všech parametrů simulace lze pomocí tlačítka \texttt{Pokračovat} přejít k dalšímu kroku, kterým je konfigurace harmonogramu vystoupení kapel. Harmonogram je možné vytvořit ručně, načíst z dříve uloženého souboru, nebo automaticky vygenerovat pomocí vestavěného generátoru. Tlačítko \texttt{Pokračovat} ve spodním panelu obrazovky je nyní deaktivované a zpřístupní se až v okamžiku vytvoření, vygenerování či načtení časového harmonogramu kapel.
...
Vybraný objekt, zónu nebo spojení mezi zónami lze smazat stiskem klávesy \texttt{Delete}. Smazáním vstupu se odstraní všechna jeho propojení. Smazáním zóny se odstraní všechny objekty uvnitř zóny i všechna její spojení. Na obrázku \ref{fig:editor_festivalovy_areal} je ukázka možného výsledného rozložení festivalového areálu.
...
\paragraph{Automatická simulace}\hfill\\
...
Canvas je jeden z nejdůležitějších widgetů Tkinteru, protože umožňuje kreslení grafických objektů, práci s tvary, obrázky a detekci událostí myši. V této práci tvoří Canvas základ editoru festivalového areálu. Uživatel na něm může umisťovat objekty, přesouvat je, vybírat, propojovat nebo upravovat jejich vlastnosti. Canvas poskytuje funkce pro práci s bounding boxy, detekci kliknutí, překreslování a manipulaci s jednotlivými prvky, což umožňuje vytvářet interaktivní vizuální editor.



Tyto prvky tvoří základní stavební kameny aplikace a umožňují vytvářet strukturované, interaktivní a vizuálně přehledné uživatelské rozhraní. 

Tkinter pracuje na jednoduchém event‑driven modelu, kdy uživatelské akce (kliknutí, pohyb myši, změna hodnoty) generují události, které aplikace zpracovává. Celé rozhraní je řízeno hlavní smyčkou mainloop(), která neustále čeká na události a zajišťuje překreslování a aktualizaci GUI.


Tyto základní widgety tvoří jádro grafického rozhraní a umožňují vytvářet strukturované, interaktivní a funkční GUI. V kombinaci s moderními prvky knihovny CustomTkinter poskytují dostatečnou flexibilitu pro tvorbu editoru i ovládacích panelů simulace.
