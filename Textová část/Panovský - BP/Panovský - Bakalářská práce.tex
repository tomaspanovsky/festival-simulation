...
Nyní si představíme uživatelské rozhraní aplikace a možnosti nastavení simulace. Na obrázku \ref{fig:uvodni_obrazovka} vidíme úvodní obrazovku aplikace, ta obsahuje spodní panel se třemi základními ovládacími prvky: \texttt{Nastavení}, \texttt{Pokračovat} a \texttt{Zavřít}. Tlačítko \texttt{Nastavení} otevře nabídku parametrů simulace, kde může uživatel upravit vstupní hodnoty, jako je počet návštěvníků nebo kolik dnů festival bude probíhat. Pomocí tlačítka \texttt{Pokračovat} se uživatel přesune k dalšímu kroku, ve kterém se nastavuje festivalový lineup. Tlačítko \texttt{Zavřít} slouží k ukončení aplikace. 

\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{images/uvodni_obrazovka.png}
    \caption{Úvodní obrazovka aplikace}
    \label{fig:uvodni_obrazovka}
\end{figure}

\subsubsection{Nastavení simulace}
Po kliku na tlačítko nastavení se otevře zkladní okno nastavení, kde uživatel může nastavit základní údaje. Toto okno je vidět na obrázku (obrázek). Základní tři základní údaje jsou Počet návštěvníků, kolik dnů bude festival trvat a v kolik simulačních hodin má simulace začít (jakože chápeš kolik hodin bude na festivalu v době začátku simulace) Od tohoto údaje se následně počítá jak dlouho simulace poběží, protože je to počet dní x 1440 protože den má 1440 minut takže simulace začne a skončí ve stejnou hodinu o zadaný počet dnů později. Ve spodní části základního nastavovacího okna se nachází tlačítka pro rozšířené možnosti nastavení.

\paragraph{Kapacity objektů }
\paragraph{Provozní doba stánků}
\paragraph{Ceny festivalu}
\paragraph{Ceny merche}
\paragraph{Označení objektů}
\subsubsection{Nastavení kapel}
\subsubsection{Nastavení rozložení festivalového areálu}
